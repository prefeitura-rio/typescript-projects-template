...
initialize_project() {
  ...
  if ! grep -q '"name": "backend"' package.json; then
    return
  fi

  require_interactive

  [[ -f package.json && -f devenv.nix ]] || die "template backend files are missing"

  read -r -p "Project name: " project_name
  validate_package_name "$project_name" || die "project name must be a valid npm package name"
  [[ "$project_name" != "backend" ]] || die "project name cannot be the template placeholder"
  ...
  sed -i "s|\"name\": \"backend\"|\"name\": \"$project_name\"|" package.json
  sed -i "s|name = \"api\"|name = \"$project_name\"|" devenv.nix
